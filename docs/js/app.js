...
var App = function () {
  ...
  var uiInit = function uiInit() {
    ...
    sScrollAlt = $('#sidebar-alt-scroll'); // Initialize sidebars functionality

    handleSidebar('init'); // Sidebar navigation functionality

    handleNav(); // Interactive blocks functionality

    interactiveBlocks(); // Scroll to top functionality

    scrollToTop(); // Template Options, change features

    templateOptions(); // Resize #page-content to fill empty space if exists (also add it to resize and orientationchange events)

    resizePageContent();
    $(window).resize(function () {
      resizePageContent();
    });
    $(window).bind('orientationchange', resizePageContent); // Add the correct copyright year at the footer
    ...
    $('.select-select2').select2(); // Initialize Bootstrap Colorpicker
    ...
    $('.input-slider').slider(); // Initialize Tags Input
  ...
  var handleNav = function handleNav() {
    ...
    var upSpeed = 250;
    var downSpeed = 250; // Get all vital links

    var menuLinks = $('.sidebar-nav-menu');
    var submenuLinks = $('.sidebar-nav-submenu'); // Primary Accordion functionality
  ...
  var handlePageScroll = function handlePageScroll(sElem, sHeightDiff, sSpeed) {
    if (!page.hasClass('disable-menu-autoscroll')) {
      var elemScrollToHeight; // If we have a static layout scroll the page
  ...
  var handleSidebar = function handleSidebar(mode, extra) {
    if (mode === 'init') {
      ...
      handleSidebar('sidebar-alt-scroll'); // Close the other sidebar if we hover over a partial one
  ...
  var resizePageContent = function resizePageContent() {
    ...
    pageContent.css('min-height', $(window).height() + 'px'); // // If we have a fixed sidebar/header layout or each sidebars’ height < window height
  ...
  var chatUi = function chatUi() {
    ...
    var chatMsg = ''; // Initialize scrolling on chat talk list
    ...
    $('#sidebar-chat-form').submit(function (e) {
      ...
      chatMsg = chatInput.val(); // If the user typed a message

      if (chatMsg) {
        // Add it to the message list
        chatMessages.append('<li class="chat-talk-msg chat-talk-msg-highlight themed-border animation-slideLeft">' + $('<div />').text(chatMsg).html() + '</li>'); // Scroll the message list to the bottom
  ...
  var templateOptions = function templateOptions() {
    ...
    var cookies = true;
    var themeColorCke; // If cookies have been enabled

    if (cookies) {
      themeColorCke = Cookies.get('optionThemeColor') ? Cookies.get('optionThemeColor') : false; // Update color theme
    ...
    $('a[data-theme="' + themeColor + '"]', colorList).parent('li').addClass('active'); // When a color theme link is clicked
  ...
  var handlePrint = function handlePrint() {
    ...
    var pageCls = page.prop('class'); // Remove all classes from #page-container

    page.prop('class', ''); // Print the page
  ...
  var previewUploadImage = function previewUploadImage() {
...
$(document).ready(function () {
  ...
  function correctUrl(path, url, context) {
    var otherSite = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
    ...
    if (sectionElement.length > 0) {
      ...
      $.ajax({
        ...
        success: function success(data) {
          $('.btn-primary').attr('disabled', false);
          $("#messageUrl").remove();
          _url = canonicalLink.val();

          if (canonicalCheckbox.prop('checked')) {
            if (canonicalLink.val() !== "/" + data + "/" + linkOriginal.val()) {
              _url = "/" + data + "/" + linkOriginal.val();
              canonicalLink.val(_url);
            }
          } else {
            canonicalLink.val(_url);

            if (verifyCorrect) {
              correctUrl("/admin/menu/isseturl", _url, context);
            }
          }
